...
#include "src/svf_peak.cpp"
#if (FAUST_INPUTS - FAUST_OUTPUTS) > 0
#error Number of inputs and outputs in faust code must match!
#endif
namespace project {
struct svf_peakMetaData {
		SN_NODE_ID("svf_peak");
};
template <int NV, class ModParameterClass=scriptnode::parameter::empty_list>
using svf_peak = scriptnode::faust::faust_static_wrapper<NV, ModParameterClass, _svf_peak, svf_peakMetaData, FAUST_OUTPUTS>;
} // namespace project
 // undef dummy macros
